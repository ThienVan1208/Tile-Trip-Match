# Tile Trip Match - Technical Design Document

## 1. System Architecture
The project utilizes the **Model-View-Presenter (MVP)** pattern combined with **Event-Driven Architecture (ScriptableObject-based)**.
...
- **Professional Standard:** This modular approach makes the project easier to Unit Test and highly maintainable for team-based development.

---

## 2. Level Data Structure
Level data is structured using **ScriptableObjects (`LevelDataSO`)**:
- **Data Schema:** Each level consists of a list of `TileSpawnInfor`, which stores Grid Coordinates (X, Y), Layer Index (Z), and Icon ID.
...
- **Memory Optimization:** Instead of loading all 10 levels into memory at startup, I implemented **Addressables** for lazy loading. Levels are loaded on-demand and released when no longer needed, significantly reducing the RAM footprint.
